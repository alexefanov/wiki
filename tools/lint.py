#!/usr/bin/env python3
"""Wiki lint helper — structural checks for ΩmegaWiki / Проверка структуры ΩmegaWiki.
...
def check_field_values(wiki_dir: Path, pages: dict[str, Path]) -> list[LintIssue]:
    ...
    for slug, fpath in pages.items():
        ...
        # Check enum fields
        enum_checks = {
            "papers": [("importance", "papers.importance"), ("source_type", "papers.source_type")],
...
def check_content_quality(wiki_dir: Path, pages: dict[str, Path]) -> list[LintIssue]:
    ...
    for slug, fpath in pages.items():
        ...
        # methods without description (warning only)
        if page_type == "methods":
            val = extract_frontmatter_value(content, "description")
            if not val:
                issues.append(LintIssue("🟡", "missing-description", rel,
                                        "methods page has no description field"))
