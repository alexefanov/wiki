#!/usr/bin/env python3
"""OmegaWiki — Wiki Knowledge Engine / Движок базы знаний Wiki.
...
Основные операции для исследовательской базы знаний на вики: чтение/запись
метаданных сущностей, управление типизированным графом, запросы состояния
знаний, контекстная компиляция, контроль жизненного цикла, аудит.
...
STOP_WORDS = frozenset({
    ...
    # Russian stop words
    "в", "во", "не", "что", "он", "на", "я", "с", "со", "как", "а", "то", "все", "она", "так", "его",
...
def slugify(title: str) -> str:
    ...
    Examples:
        ...
        >>> slugify("Обследование зданий и сооружений")
        'обследование-зданий-и-сооружений'
    """
    # Normalize: lowercase, replace non-alphanum with spaces (including Cyrillic)
    text = re.sub(r"[^a-zа-яё0-9\s]", " ", title.lower())
    words = text.split()
...
def get_maturity(wiki_root: str, as_json: bool = False) -> dict:
    ...
    has_experiment_evidence = any(
        e.get("type") in ("supports", "invalidates", "confirms", "contradicts")
...
def rebuild_index(wiki_root: str) -> None:
    ...
    for entity_type in ENTITY_DIRS:
        ...
        for f in sorted(entity_dir.glob("*.md")):
            ...
            if "number" in fm and fm["number"]:
                line_parts.append(f'    number: "{fm["number"]}"')
            if "oks" in fm and fm["oks"]:
                line_parts.append(f'    oks: "{fm["oks"]}"')
            if "udk" in fm and fm["udk"]:
                line_parts.append(f'    udk: "{fm["udk"]}"')
            if "bbk" in fm and fm["bbk"]:
                line_parts.append(f'    bbk: "{fm["bbk"]}"')
            if "source_type" in fm and fm["source_type"]:
                line_parts.append(f'    source_type: "{fm["source_type"]}"')
